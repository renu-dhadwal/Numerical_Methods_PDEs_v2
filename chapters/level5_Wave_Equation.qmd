...
### 2️⃣ What happens to the characteristics?
...
### 3️⃣ Visualizing the moving shock
...
rho_L, rho_R = 0.6, 0.2
c_L, c_R = 1 - 2*rho_L, 1 - 2*rho_R
s = (rho_L*(1 - rho_L) - rho_R*(1 - rho_R)) / (rho_L - rho_R)

x0_left = np.linspace(-2, 0, 12)
x0_right = np.linspace(0, 2, 12)
...
for x0 in x0_left:
    fig.add_trace(go.Scatter(x=x0 + c_L*t, y=t, mode='lines',
                             line=dict(color='royalblue', width=1), showlegend=False))
# Right characteristics
for x0 in x0_right:
    fig.add_trace(go.Scatter(x=x0 + c_R*t, y=t, mode='lines',
                             line=dict(color='royalblue', width=1), showlegend=False))

# Shock line
fig.add_trace(go.Scatter(x=s*t, y=t, mode='lines',
                         line=dict(color='red', width=3), name=f'Shock: s={s:.2f}'))

fig.update_layout(
    title=f"Moving Shock (ρ_L={rho_L}, ρ_R={rho_R},  s={s:.2f})",
