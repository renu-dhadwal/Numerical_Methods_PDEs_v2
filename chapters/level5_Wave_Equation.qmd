...
#| echo: false
#| warning: false
#| fig-cap: "Interactive plot: adjust densities to see shocks or rarefactions form."
...
def plot_characteristics(rho_L=0.2, rho_R=0.8):
    ...
    for i, x0 in enumerate(x0_vals):
        ...
        fig.add_trace(go.Scatter(
            x=x, y=t, mode='lines', line=dict(width=1, color='royalblue'), showlegend=False
        ))
    
    # Plot shock line
    x_shock = s * t
    fig.add_trace(go.Scatter(
        x=x_shock, y=t, mode='lines', line=dict(color='red', width=3), name='Shock'
    ))
    
    # Annotations
    fig.update_layout(
        title=f"ρ_L = {rho_L:.2f}, ρ_R = {rho_R:.2f}, Shock speed s = {s:.2f}",
        xaxis_title="x",
        yaxis_title="t",
        yaxis=dict(autorange='reversed', range=[2,0]),
...
# Interactive sliders
interact(plot_characteristics,
         rho_L=widgets.FloatSlider(value=0.2, min=0.0, max=1.0, step=0.05, description='ρ_L'),
         rho_R=widgets.FloatSlider(value=0.8, min=0.0, max=1.0, step=0.05, description='ρ_R'));
```