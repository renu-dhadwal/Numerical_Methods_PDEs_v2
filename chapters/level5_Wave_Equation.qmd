...
def make_preset_frames(rho_L, rho_R, preset_idx, t_max=3.0, n_frames=28, n_char=40):
    ...
    for k, tk in enumerate(t_frames):
        ...
        if compressive:
            # Shock case
            # Left characteristics: start at x0 < 0, move with speed c_L
            x0_left = np.linspace(-2.0, 0, n_char)
            for x0 in x0_left:
            ...
            # Right characteristics: start at x0 > 0, move with speed c_R
            x0_right = np.linspace(0, 2.0, n_char)
            for x0 in x0_right:
                ts = np.linspace(0, tk, 18)
                xs = x0 + c_R * ts
                traces.append(go.Scatter(x=xs, y=ts, mode="lines",
            ...
            # Shock line (red) - starts at x=0, t=0
            ts_shock = np.linspace(0, tk, 50)
            xs_shock = s * ts_shock
            traces.append(go.Scatter(x=xs_shock, y=ts_shock, mode="lines",
                                     line=dict(color="red", width=3),
                                     name=f"shock speed s={s:.3f}"))
        else:
            # Rarefaction case
            # Left characteristics: start at x0 < 0, move with speed c_L
            x0_left = np.linspace(-2.0, 0, n_char//2)
            for x0 in x0_left:
                ts = np.linspace(0, tk, 18)
                xs = x0 + c_L * ts
                traces.append(go.Scatter(x=xs, y=ts, mode="lines",
            ...
            # Right characteristics: start at x0 > 0, move with speed c_R
            x0_right = np.linspace(0, 2.0, n_char//2)
            for x0 in x0_right:
                ts = np.linspace(0, tk, 18)
                xs = x0 + c_R * ts
                traces.append(go.Scatter(x=xs, y=ts, mode="lines",
            ...
            # Rarefaction fan: characteristics emanating from x=0, t=0 with varying speeds
            ...
            traces.append(go.Scatter(x=c_L * np.linspace(0, tk, 2), y=np.linspace(0, tk, 2),
                                     mode="lines", line=dict(color="darkgreen", width=2.5, dash="dash"),
                                     name=f"c_L={c_L:.2f}"))
...
for fr in all_frames:
    ...
    if n_missing > 0:
        fr.data += tuple(go.Scatter(x=[None], y=[None], visible=False) for _ in range(n_missing))
...
fig.update_layout(
    title=preset_meta[default_idx]["label"] + " — Riemann presets (CORRECTED)",
    xaxis_title="x",
    yaxis_title="t (time ↑)",
