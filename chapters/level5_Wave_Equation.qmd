...
def make_preset_frames(rho_L, rho_R, preset_idx, t_max=3.0, n_frames=28, n_char=50):
    """
    Build frames for a single preset.
    n_char controls how many characteristic lines per side (total ~2*n_char for compressive).
    """
    f = lambda r: r*(1-r)
    a = lambda r: 1 - 2*r
    fL, fR = f(rho_L), f(rho_R)
    # shock speed (or char speed if equal)
    s = (fL - fR) / (rho_L - rho_R) if not np.isclose(rho_L, rho_R) else a(rho_L)
    c_L, c_R = a(rho_L), a(rho_R)
    compressive = (c_L > c_R)

    # sample initial x0 *near the discontinuity* so intersections are visible
    # use smaller interval around x=0 (e.g., [-1,0) and (0,1]) rather than huge [-2.5,2.5]
    x0_left = np.linspace(-1.0, -0.001, n_char)
    x0_right = np.linspace( 0.001,  1.0, n_char)
    ...
    for k, tk in enumerate(t_frames):
        traces = []
        if compressive:
            # draw left family characteristics (from negative x0 towards right)
            for x0 in x0_left:
                ...
                traces.append(go.Scatter(x=xs, y=ts, mode='lines',
                                         ...
                                         showlegend=False))
            # draw right family characteristics
            for x0 in x0_right:
                ts = np.linspace(0, tk, 18)
                xs = x0 + c_R * ts
                traces.append(go.Scatter(x=xs, y=ts, mode='lines',
                                         line=dict(color='royalblue', width=0.9),
                                         showlegend=False))
            # shock line (make it the last VISIBLE trace so trace-slot ordering is stable)
            shock_x = s * np.linspace(0, tk, 2)
            shock_t = np.linspace(0, tk, 2)
            traces.append(go.Scatter(x=shock_x, y=shock_t, mode='lines',
                                     line=dict(color='red', width=3), name=f"s={s:.3f}"))
        else:
            # rarefaction: many characteristic slopes filling fan between c_L and c_R
            ...
            # bounding characteristics as dashed lines (put after the family)
...
# Precompute frames for all presets and collect metadata
all_frames = []
preset_meta = []
for i, (label, rL, rR) in enumerate(presets):
    frames_i, s_i, comp_i, t_frames = make_preset_frames(rL, rR, i, t_max=3.0, n_frames=28, n_char=60)
...
# Find maximum traces in any frame and pad every frame to that length
max_traces = max(len(fr.data) for fr in all_frames)
for fr in all_frames:
    # number of visible traces in this frame
    n_present = len(fr.data)
    n_missing = max_traces - n_present
    if n_missing:
        # append invisible placeholders to occupy trace slots
        for _ in range(n_missing):
            fr.data += (go.Scatter(x=[None], y=[None], mode="lines", visible=False),)

# Build figure and initialize with default preset first frame
default_idx = 0
first_frame_name = preset_meta[default_idx]["first_frame_name"]
first_frame = next(fr for fr in all_frames if fr.name == first_frame_name)

fig = go.Figure(frames=all_frames)
fig.add_traces(first_frame.data)  # these are padded traces

# Build slider for the default preset
default_frames = preset_meta[default_idx]["frame_names"]
steps = []
for k, name in enumerate(default_frames):
...
# Updatemenus: Play/Pause and a preset dropdown that animates that preset's full sequence
updatemenus = [
    dict(type="buttons", direction="left", x=0.02, y=0.95, xanchor="left", yanchor="top",
         buttons=[
             dict(label="Play", method="animate",
                  args=[default_frames, {"frame": {"duration": 80, "redraw": True}, "fromcurrent": True}]),
             dict(label="Pause", method="animate",
                  args=[[None], {"frame": {"duration": 0, "redraw": False}}])
         ]),
    dict(type="buttons", direction="down", x=1.02, y=0.8, xanchor="left", yanchor="top",
         buttons=[
             dict(label=meta["label"],
                  method="animate",
