...
def make_preset_frames(rho_L, rho_R, preset_idx, t_max=3.0, n_frames=28, n_x0=101):
    """Return a list of frames for a preset. Frame names: f'p{preset_idx}_f{frame_index}'."""
    f = lambda r: r*(1-r)
    a = lambda r: 1 - 2*r
    ...
    for k, tk in enumerate(t_frames):
        traces = []
        if compressive:
            # left & right characteristic families
            for x0 in x0_left:
            ...
            # rarefaction fan (many characteristics)
...
# default preset index to display at load
default_idx = 0
...
# Add initial traces from default preset's first frame explicitly
first_frame = next(fr for fr in all_frames if fr.name == preset_meta[default_idx]["first_frame_name"])
fig.add_traces(first_frame.data)

# Build slider for the default preset (time control)
steps = []
...
# Updatemenus: Play/Pause and Preset selector
updatemenus = [
    # Play/Pause buttons
    dict(type="buttons", direction="left", x=0.02, y=0.95, xanchor="left", yanchor="top",
         buttons=[
             dict(label="Play",
                  method="animate",
                  # when Play is clicked, animate the currently visible preset's frames.
                  args=[default_frames, {"frame": {"duration": 80, "redraw": True}, "fromcurrent": True}]),
             dict(label="Pause",
                  method="animate",
                  args=[[None], {"frame": {"duration": 0, "redraw": False}}])
         ]),
    # Preset dropdown (animates through the full preset frames when a preset is chosen)
