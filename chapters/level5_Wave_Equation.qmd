...
# --- parameters (tweak these for speed/quality) ---
t_max = 2.5
n_frames = 40          # number of animation frames (time steps)
n_x0 = 201             # number of initial points (characteristics)
pts_per_char = 18      # points to draw per characteristic segment

# initial Gaussian u(x,0) = exp(-100 (x-0.3)^2)
def u0(x): return np.exp(-100.0*(x-0.3)**2)
a_of = lambda u: 1 - 2*u   # characteristic speed a(u) = f'(u)

# sample initial axis
x0 = np.linspace(-3.0, 3.0, n_x0)
u0_vals = u0(x0)
a_vals = a_of(u0_vals)

# compute pairwise neighbour intersections (approx envelope)
dx = np.diff(x0)
da = np.diff(a_vals)
with np.errstate(divide='ignore', invalid='ignore'):
    t_candidates = dx / (-da)
valid = (t_candidates > 1e-8) & np.isfinite(t_candidates)
idx_pairs = np.where(valid)[0]
t_int = t_candidates[idx_pairs]
x_int = x0[idx_pairs] + a_vals[idx_pairs]*t_int

# sort envelope points by time
order = np.argsort(t_int)
t_env = t_int[order]
x_env = x_int[order]
earliest_exists = len(t_env) > 0
t_earliest = t_env[0] if earliest_exists else None
x_earliest = x_env[0] if earliest_exists else None
...
for tk in t_frames:
    traces = []
    # characteristics (short polylines up to tk)
    t_seg = np.linspace(0, tk, pts_per_char)
    for xi, ai in zip(x0, a_vals):
        xs = xi + ai * t_seg
        traces.append(go.Scatter(x=xs, y=t_seg, mode='lines',
                                 line=dict(color='royalblue', width=0.6), showlegend=False))
    # intersection markers up to tk
    if earliest_exists:
        mask = t_int <= tk
        if mask.any():
            traces.append(go.Scatter(x=x_int[mask], y=t_int[mask], mode='markers',
                                     marker=dict(color='orange', size=6), name='Pairwise intersections'))
            # envelope curve so far
            mask_env = t_env <= tk
            traces.append(go.Scatter(x=x_env[mask_env], y=t_env[mask_env],
                                     mode='lines', line=dict(color='orange', width=2), name='Envelope (approx)'))
            # earliest collision
            traces.append(go.Scatter(x=[x_earliest], y=[t_earliest],
                                     mode='markers+text',
                                     marker=dict(symbol='x', size=12, color='black'),
                                     text=[f"First collision<br>t={t_earliest:.3f}, x={x_earliest:.3f}"],
                                     textposition="top right", showlegend=False))
    frames.append(go.Frame(data=traces, name=str(round(tk,6))))
...
# play/pause inside figure (ensure visible)
updatemenus = [
...
title = "Characteristics & Approximate Envelope — Gaussian initial data u(x,0)=exp(-100(x-0.3)^2)"
if earliest_exists:
    title += f" — earliest t={t_earliest:.3f}"
...
# show (Plotly-only; will embed Play & slider in the HTML)
fig.show()
```