...
#| echo: false
#| warning: false
# interactive shock/rarefaction explorer (Plotly + ipywidgets)
import numpy as np
...
def make_figure(rho_L, rho_R, t_max=2.0):
    # Prepare grid
    ...
    if np.isclose(rho_L, rho_R):
        s = 1 - 2 * rho_L   # same state -> characteristic speed
    else:
    ...
    for i, x0 in enumerate(x0_vals):
        x = x0 + c_vals[i] * t
        fig.add_trace(go.Scatter(x=x, y=t, mode='lines',
                                 line=dict(color='royalblue', width=1),
                                 showlegend=False))

    # Shock trajectory (red)
    fig.add_trace(go.Scatter(x=s * t, y=t, mode='lines',
                             line=dict(color='red', width=3),
                             name=f's(t): s={s:.3f}'))

    # Layout (flip y so origin at bottom visually)
    fig.update_layout(
        title=f'Characteristics (ρ_L={rho_L:.2f},  ρ_R={rho_R:.2f}) — shock speed s={s:.3f}',
        xaxis_title='x',
        yaxis_title='t',
...
# Widgets
rho_L_slider = widgets.FloatSlider(value=0.2, min=0.0, max=1.0, step=0.01, description='ρ_L')
rho_R_slider = widgets.FloatSlider(value=0.8, min=0.0, max=1.0, step=0.01, description='ρ_R')
tmax_slider  = widgets.FloatSlider(value=2.0, min=0.5, max=5.0, step=0.1, description='t_max')

out = widgets.Output()

def update_plot(change=None):
    # clear and draw into the output widget
    with out:
        clear_output(wait=True)
        fig = make_figure(rho_L_slider.value, rho_R_slider.value, t_max=tmax_slider.value)
        fig.show()

# Attach observers
rho_L_slider.observe(update_plot, names='value')
rho_R_slider.observe(update_plot, names='value')
tmax_slider.observe(update_plot, names='value')

# Display controls + initial figure
