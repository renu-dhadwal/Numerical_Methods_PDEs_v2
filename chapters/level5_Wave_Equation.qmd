...
def make_preset_frames(rho_L, rho_R, preset_idx, t_max=3.0, n_frames=28, n_char=40):
    ...
    for k, tk in enumerate(t_frames):
        ...
        if compressive:
            # Shock case
            # Left characteristics (start left of x=0)
            x0_left = np.linspace(-1.0, -0.05, n_char)
            for x0 in x0_left:
                ...
                traces.append(go.Scatter(x=xs, y=ts, mode="lines",
                                         ...
                                         showlegend=False))
            # Right characteristics (start right of x=0)
            x0_right = np.linspace(0.05, 1.0, n_char)
            for x0 in x0_right:
                ts = np.linspace(0, tk, 18)
                xs = x0 + c_R * ts
                traces.append(go.Scatter(x=xs, y=ts, mode="lines",
                                         ...
                                         showlegend=False))
            # Shock line (red)
            xs = s * np.linspace(0, tk, 2)
            ts = np.linspace(0, tk, 2)
            traces.append(go.Scatter(x=xs, y=ts, mode="lines",
                                     line=dict(color="red", width=3),
