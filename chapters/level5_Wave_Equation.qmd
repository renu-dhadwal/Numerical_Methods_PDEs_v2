...
def build_animated_figure(rho_L, rho_R, t_max=3.0, n_frames=40, n_x0=61):
    # compute speeds and shock speed
    c_L = a(rho_L); c_R = a(rho_R)
    ...
    # prepare x0 sampling for characteristics (separate left/right so rays are clear)
    ...
    for k, tk in enumerate(t_frames):
        traces = []
        if compressive:
            # characteristics from left and right up to tk
            for x0 in x0_left:
                xs = x0 + c_L * np.linspace(0, tk, 30)
                ts = np.linspace(0, tk, 30)
                traces.append(go.Scatter(x=xs, y=ts, mode='lines', line=dict(color='royalblue', width=0.6), showlegend=False))
            for x0 in x0_right:
                xs = x0 + c_R * np.linspace(0, tk, 30)
                ts = np.linspace(0, tk, 30)
                traces.append(go.Scatter(x=xs, y=ts, mode='lines', line=dict(color='royalblue', width=0.6), showlegend=False))
            # shock line up to tk
            ...
            # rarefaction fan: characteristics issuing from origin with slopes between c_L and c_R
            slopes = np.linspace(c_L, c_R, 41)
            for xi in slopes:
                xs = xi * np.linspace(0, tk, 30)
                ts = np.linspace(0, tk, 30)
                traces.append(go.Scatter(x=xs, y=ts, mode='lines', line=dict(color='green', width=0.9), showlegend=False, opacity=0.8))
    ...
    steps = [
        dict(method="animate",
             ...
             label=f"{t_frames[i]:.2f}")
        for i in range(len(t_frames))
    ]
    sliders = [dict(active=0, pad={"t": 30}, steps=steps, currentvalue={"prefix":"t = "})]

    # updatemenus play/pause
    updatemenus = [
    ...
    regime = "Shock" if compressive else "Rarefaction"
    title = f"Riemann: ρ_L={rho_L:.3f}, ρ_R={rho_R:.3f} — {regime} (s={s:.3f})" if compressive else f"Riemann: ρ_L={rho_L:.3f}, ρ_R={rho_R:.3f} — {regime}"
...
def update(change=None):
    with out:
        ...
        fig = build_animated_figure(rho_L_slider.value, rho_R_slider.value, t_max=tmax_slider.value, n_frames=50)
        fig.show()
