...
def make_preset_frames(rho_L, rho_R, t_max=3.0, n_frames=36, n_x0=121):
    f = lambda r: r*(1-r)
    a = lambda r: 1 - 2*r
    ...
    compressive = (c_L > c_R)
    x0_left = np.linspace(-3, -0.01, n_x0//2)
    x0_right = np.linspace(0.01, 3, n_x0//2)
    t_frames = np.linspace(0, t_max, n_frames)
    frames = []
    for tk in t_frames:
        traces = []
        if compressive:
            for x0 in x0_left:
                xs = x0 + c_L * np.linspace(0, tk, 20)
                ts = np.linspace(0, tk, 20)
                traces.append(go.Scatter(x=xs, y=ts, mode='lines', line=dict(color='royalblue', width=0.7), showlegend=False))
            for x0 in x0_right:
                xs = x0 + c_R * np.linspace(0, tk, 20)
                ts = np.linspace(0, tk, 20)
                traces.append(go.Scatter(x=xs, y=ts, mode='lines', line=dict(color='royalblue', width=0.7), showlegend=False))
            traces.append(go.Scatter(x=s * np.linspace(0, tk, 2), y=np.linspace(0, tk, 2),
                                     mode='lines', line=dict(color='red', width=3), name=f's={s:.3f}'))
        else:
            slopes = np.linspace(c_L, c_R, 41)
            for xi in slopes:
                xs = xi * np.linspace(0, tk, 20)
                ts = np.linspace(0, tk, 20)
                traces.append(go.Scatter(x=xs, y=ts, mode='lines', line=dict(color='green', width=0.8), showlegend=False, opacity=0.9))
            traces.append(go.Scatter(x=c_L * np.linspace(0, tk, 2), y=np.linspace(0, tk, 2), mode='lines',
                                     line=dict(color='green', width=2, dash='dash'), name=f'c_L={c_L:.3f}'))
            traces.append(go.Scatter(x=c_R * np.linspace(0, tk, 2), y=np.linspace(0, tk, 2), mode='lines',
                                     line=dict(color='green', width=2, dash='dash'), name=f'c_R={c_R:.3f}'))
        frames.append(go.Frame(data=traces, name=str(tk)))
    return frames, s, compressive

# Build frames for each preset (precompute)
all_preset_frames = []
meta = []
for label, rL, rR in presets:
    frames, s, comp = make_preset_frames(rL, rR, t_max=3.0, n_frames=36, n_x0=121)
    all_preset_frames.append(frames)
    meta.append((label, rL, rR, s, comp))

# We'll show the first preset by default
default_idx = 0
fig = go.Figure(frames=all_preset_frames[default_idx])
fig.add_traces(all_preset_frames[default_idx][0].data)

# slider for time (shared)
t_vals = [float(fr.name) for fr in all_preset_frames[0]]
steps = [dict(method="animate", args=[[str(i)], dict(mode="immediate", frame=dict(duration=60, redraw=True))], label=f"{t_vals[i]:.2f}") for i in range(len(t_vals))]
sliders = [dict(active=0, pad={"t":40}, steps=steps, currentvalue={"prefix":"t = "})]

# Updatemenu: preset buttons (switch to preset k by replacing frames and initial traces)
buttons = []
for k, (label, rL, rR, s, comp) in enumerate(meta):
    # When clicked, animate to frame 0 of that preset (we use method 'animate' with args = [frames_names]
    # but easier: set visible traces via 'restyle' is complex; we use `relayout` + custom frames by calling animate to the first frame name
    # Simpler: use args that update title and replace data by the 0th frame of that preset.
    btn = dict(
        label=label,
        method="animate",
        args = [
            [all_preset_frames[k][0].name],  # animate to first frame of preset k
...
fig.update_layout(
    title=f"{meta[default_idx][0]} — Riemann presets",
    xaxis_title="x",
    yaxis_title="t (time ↑)",
    ...
    updatemenus=[
        ...
        dict(type="buttons", direction="down", x=1.02, y=0.8, xanchor="left", yanchor="top",
             buttons=[dict(label=meta[i][0], method="animate", args=[[all_preset_frames[i][0].name], dict(mode="immediate", frame=dict(duration=0))]) for i in range(len(meta))])
    ],
