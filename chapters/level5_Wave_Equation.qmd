...
# Precompute frames and metadata
all_frames = []
preset_meta = []
...
# Determine maximum number of traces any frame has
max_traces = max(len(fr.data) for fr in all_frames)

# Pad every frame's data list to length max_traces with invisible placeholders
for fr in all_frames:
    n_missing = max_traces - len(fr.data)
    if n_missing > 0:
        for _ in range(n_missing):
            fr.data += (go.Scatter(x=[], y=[], mode="lines", visible=False),)

# Build figure and initialize with padded first frame (default preset)
default_idx = 0
...
# Add padded traces from the first frame to fig.data
fig.add_traces(first_frame.data)

# Build slider for default preset
...
# Updatemenus: Play/Pause and Preset selector (each preset animates through its own frames)
updatemenus = [
