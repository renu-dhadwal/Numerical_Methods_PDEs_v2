...
for k, tk in enumerate(t_frames):
    ...
    for i, x0 in enumerate(x0_vals):
        ...
        data.append(go.Scatter(x=xs, y=ts, mode='lines', line=dict(color='royalblue', width=1), showlegend=False))
    # shock speed (Rankine-Hugoniot)
    if np.isclose(rho_L, rho_R):
        s = 1 - 2*rho_L
    else:
        s = (f(rho_L) - f(rho_R)) / (rho_L - rho_R)
    data.append(go.Scatter(x=[s, s], y=[0, tk], mode='lines', line=dict(color='red', width=3), name='Shock'))
...
fig.update_layout(
    title=f"Characteristics (rho_L={rho_L:.2f}, rho_R={rho_R:.2f})",
    xaxis_title="x",
    yaxis_title="t",
    yaxis=dict(autorange='reversed', range=[t_max, 0]),
    sliders=sliders,
...
**Notes for A**
- If `quarto render` hangs on this chunk in your environment, either:
  - render interactively (e.g., `quarto preview` or open the page in Jupyter/VSCode), or
  - temporarily set chunk option `#| eval: false` to skip execution during render and keep the interactive block for live pages, or
  - use block B below (Plotly-only) for robust headless rendering.
...
#| echo: false
#| warning: false
# Plotly-only moving-shock explorer (dropdown presets + time slider)
# Robust for headless `quarto render`. Use edit-in-code to change presets or add more.
...
presets = [
    ...
    ("Left-moving shock (0.2,0.6)", 0.2, 0.6)
]

# pick preset index to show initially
preset_index = 0
rho_L, rho_R = presets[preset_index][1], presets[preset_index][2]
...
# Build frames for the chosen preset
frames = []
for k, tk in enumerate(t_frames):
    data = []
    for i, x0 in enumerate(x0_vals):
...
# dropdown for presets (simple: switch title & users can edit presets in code)
updatemenus = [
    dict(type="buttons",
         direction="down",
         showactive=True,
         ...
         xanchor="left",
         y=0.8,
         buttons=[dict(label=presets[i][0],
                       method="animate",
                       args=[[str(0)], dict(frame={"duration": 1, "redraw": True}, mode="immediate")]) for i in range(len(presets))])
]

fig.update_layout(
    title=f"Characteristics & Moving Shock — preset: {presets[preset_index][0]}",
    xaxis_title="x",
    yaxis_title="t",
    yaxis=dict(autorange='reversed', range=[t_max, 0]),
    sliders=sliders,
