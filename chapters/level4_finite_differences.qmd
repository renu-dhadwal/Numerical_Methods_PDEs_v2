...
```{python}
# Interactive Plotly figures for FTCS and Backward Euler eigenvalues
import numpy as np
import plotly.graph_objects as go

# Parameters
N = 60  # number of interior points for FTCS
m = 60  # use m for backward-euler modes (m-1 modes)
...
# Parameter grids for sliders
r_values = np.linspace(0.0, 0.8, 41)       # for FTCS (r = alpha*dt/h^2)
lambda_values = np.concatenate((np.linspace(0.01, 1, 25), np.linspace(1.5, 20, 25)))  # for BE

# Precompute eigenvalue datasets
...
# FTCS figure with slider for r
fig_ftcs = go.Figure()
fig_ftcs.add_trace(go.Scatter(x=k_ftcs, y=mu_ftcs_data[0], mode='lines+markers', name=f"r={r_values[0]:.2f}"))
...
sliders = [dict(active=0, currentvalue={"prefix": "r: "}, pad={"t": 50}, steps=steps)]

fig_ftcs.update_layout(
    title=f"FTCS eigenvalues μ_k (r={r_values[0]:.2f})",
...
# Backward Euler figure with slider for lambda
fig_be = go.Figure()
fig_be.add_trace(go.Scatter(x=i_be, y=mu_be_data[0], mode='lines+markers', name=f"λ={lambda_values[0]:.2f}"))
...
for idx, lam in enumerate(lambda_values):
    ...
    steps_be.append(step)

sliders_be = [dict(active=0, currentvalue={"prefix": "λ: "}, pad={"t": 50}, steps=steps_be)]

fig_be.update_layout(
    title=f"Backward Euler eigenvalues μ_i (λ={lambda_values[0]:.2f})",
