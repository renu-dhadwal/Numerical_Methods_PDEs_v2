...
$\dfrac{U^{n+1} - U^n}{\Delta t} = \alpha^2 A U^{n+1},$  
where $A$ is the discrete Laplacian matrix.

Rearranging:  
$U^{n+1} = (I - \lambda A)^{-1} U^n, 
\quad \lambda = \dfrac{\alpha \Delta t}{h^2}.$

The eigenvalues of $A$ (with Dirichlet BCs) are:  
$\lambda_i(A) = -4 \sin^2\!\left(\dfrac{i\pi}{2m}\right), 
\quad i=1,2,\dots,m-1.$

Eigenvalues of the iteration matrix
Since eigenvalues transform under matrix inverses:  
$\mu_i = \dfrac{1}{1 - \lambda \lambda_i(A)} 
= \dfrac{1}{1 + 4\lambda \sin^2\!\left(\dfrac{i\pi}{2m}\right)}.$
