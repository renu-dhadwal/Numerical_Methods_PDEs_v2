...
### Why the FTCS stability condition is $r \leq \tfrac{1}{2}$

If the update is 
$$
u^{(n+1)} = A u^{(n)},
$$ 
and the initial error is $e^0$, then the error after $n$ steps is
$$
e^{(n)} = A^n e^0.
$$

Using an induced matrix norm we have
$$
...
For the FTCS tridiagonal matrix $A$ (which is symmetric), the spectral radius can be computed explicitly:
$$
\mu_k = 1-4 \lambda (\sin (\frac{1 \pi}{2 N}))^2 {\Big),
\qquad k=1,\dots,N-1.
$$

$$
\rho(A) = \max_{1\leq i \leq {N-1}} |1-4\lambda (\sin (\frac{1 \pi}{2 N}))^2| \leq 1
$$

This simplifies to \\
$$
0 \leq \lambda (\sin (\frac{1 \pi}{2 N}))^2 \leq \frac{1}{2}
$$
for each $i = 1, 2 \ldots N-1$

Stability requires that the abive inequality holds for $h\rightarrow 0$ or, equivalently, as $N \rightarrow \infty$.\\
Since, we know that,
$$
\lim_{N \rightarrow \infty}[sin (\frac{(m-1)\pi}{2m})]^2 = 1
$$
, stability will occur if $0\leq \lambda \leq \frac{1}{2}$.

Thus, the **FTCS scheme is stable if and only if $r \le \tfrac{1}{2}$.**
:::
