...
<details>
<summary><b>Backward Euler Stability Analysis</b></summary>

### 1. Backward Euler scheme for Heat Equation
We discretize the 1D heat equation
\[
u_t = \alpha u_{xx}
\]
with spatial grid spacing \(h = L/(m+1)\) and timestep \(\Delta t\).

The **Backward Euler (implicit) scheme** is:
\[
\frac{U^{n+1} - U^n}{\Delta t} = \alpha A U^{n+1},
\]
...
\quad \lambda = \frac{\alpha \Delta t}{h^2}.
\]

---

### 2. Eigenvalues of the discrete Laplacian
The eigenvalues of \(A\) (with Dirichlet BCs) are:
\[
...
Since eigenvalues transform under matrix inverses:
\[
\mu_i = \frac{1}{1 - \lambda \lambda_i(A)} 
= \frac{1}{1 + 4\lambda \sin^2\!\left(\frac{i\pi}{2m}\right)}.
\]

---

### 4. Stability check
- The denominator is always greater than 1 (since \(\lambda>0\) and \(\sin^2\ge0\)).  
- Therefore:
\[
...
- In contrast, FTCS needed \(\lambda \le 1/2\) for stability.  

</details>

This will give you **zoomable, hoverable, toggleable curves** directly in your Quarto site.

---

## 2. Adding a **slider for λ**
You can go further and let the reader **move a slider** to change \(\lambda\):
...
lambda_vals = np.linspace(0.1, 10, 50)

# Precompute
mu_data = [
    1.0 / (1.0 + 4.0 * lam * (np.sin(theta)**2)) 
    for lam in lambda_vals
]

# Make figure with slider
fig = go.Figure()

# Initial plot
fig.add_trace(go.Scatter(x=i, y=mu_data[0], mode='lines+markers'))
