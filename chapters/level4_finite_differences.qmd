...
$$
Bring $U^{n+1}$ terms to the left to obtain the matrix form
$$
\boxed{\;\Big(I-\tfrac{\lambda}{2}A\Big)U^{n+1} = \Big(I+\tfrac{\lambda}{2}A\Big)U^n\;},
$$
with truncation errors $O(\Delta t^3)$ (time quadrature) and $O(\Delta t\,\Delta x^2)$ (spatial discretization of each $u_{xx}$). After dividing by $\Delta t$ and using standard consistency+stability arguments, CN is second-order in time and second-order in space (global error $O(\Delta t^2+\Delta x^2)$).
